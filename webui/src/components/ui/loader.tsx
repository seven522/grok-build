    ...
    <div className={cn("flex items-center gap-1.5", className)} aria-hidden="true">
      {[0, 1, 2].map((index) => (
        <motion.div
          ...
          className="size-2 rounded-full border border-border bg-gradient-to-b from-primary/80 to-primary/40"
        />
      ))}
    </div>
  );
    ...
    >
      {text.split("").map((char, i) => (
        <motion.span
          ...
          aria-hidden="true"
          initial={{ scale: 1, opacity: reduceMotion ? 1 : 0.5 }}
          animate={reduceMotion ? { scale: 1, opacity: 1, textShadow: "none" } : {
            scale: [1, 1.1, 1],
            textShadow: [
